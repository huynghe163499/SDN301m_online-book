...
export default function CardInfo(props) {
  ...
  useEffect(() => {
    ...
    if (info) {
      bottomFade.style.display = info.scrollHeight - info.clientHeight > info.scrollTop ? 'block' : 'none'
      info.onscroll = (e) => {
        if (e.target.scrollHeight - e.target.clientHeight === Math.round(e.target.scrollTop)) bottomFade.style.display = 'none'
  ...
  return (
    <div className={styles.cardWrapper}>
      <div className={styles.headerInfo}>
        ...
        <span className={styles.btnClose}>
          <Tooltip placement="bottom" mouseLeaveDelay={0} title="Đóng">
            <Button btnType="btn-circle" shape="circle" iconName="remove" onClick={onClose} />
          </Tooltip>
        </span>
      </div>
      ...
      <div className={`scroll-zone ${styles.info}`} id="scroll-zone">
        {useMemo(
          () => (
            <div className={`scroll-zone ${className} ${styles.detailCard}`}>{children}</div>
          ),
          [children]
        )}
      </div>
